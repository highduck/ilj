...
function findEdge(t: Tri, p1: Point, p2: Point): number {
  ...
  if (p1 == _0) {
    if (p2 == Tri_POINTS[t + 1]) {
      return 2;
    }
    if (p2 == Tri_POINTS[t + 2]) {
      return 1;
    }
    return -1;
  }
...
function clearDelaunayEdges(t: Tri) {
  // FLAGS[this + 1] = 0;
  Tri_FLAGS[t] =
    Tri_FLAGS[t] & ~(MASK_DELAUNAY_EDGE_0 | MASK_DELAUNAY_EDGE_1 | MASK_DELAUNAY_EDGE_2);
}
...
function orient2d(pa: Point, pb: Point, pc: Point): Orientation {
  const val =
    (Point_X[pa] - Point_X[pc]) * (Point_Y[pb] - Point_Y[pc]) -
 ...
 * </pre>
 */
function rotateTrianglePairFromIndices(
  t: Tri,
  p: Point,
  i: number,
  ot: Tri,
  ...
  if (n1 != 0) {
    markNeighborTriangle(ot, n1);
  }
  if (n2 != 0) {
    markNeighborTriangle(t, n2);
  }
  if (n3 != 0) {
    markNeighborTriangle(t, n3);
  }
  if (n4 != 0) {
    markNeighborTriangle(ot, n4);
  }
...
class Triangulator {
  ...
  mapTriangleToNodes(triangle: Tri) {
    if (Tri_NEIGHBORS[triangle + 0] == 0) {
      this.front_locatePoint_and_set_neighbor(triangle, Tri_POINTS[triangle + 2]);
    }
    if (Tri_NEIGHBORS[triangle + 1] == 0) {
      this.front_locatePoint_and_set_neighbor(triangle, Tri_POINTS[triangle + 0]);
    }
    if (Tri_NEIGHBORS[triangle + 2] == 0) {
      this.front_locatePoint_and_set_neighbor(triangle, Tri_POINTS[triangle + 1]);
    }
  }
  ...
  meshClean(t: Tri) {
    ...
    while (size > 0) {
      ...
      if ((flags & MASK_CONSTRAINED_EDGE_0) == 0) {
        tmp[size++] = Tri_NEIGHBORS[t];
      }
      if ((flags & MASK_CONSTRAINED_EDGE_1) == 0) {
        tmp[size++] = Tri_NEIGHBORS[t + 1];
      }
      if ((flags & MASK_CONSTRAINED_EDGE_2) == 0) {
        tmp[size++] = Tri_NEIGHBORS[t + 2];
      }
    }
  }
  ...
  edgeEventByEdge(q: Point, p: Point, node: PNode) {
    ...
    if (isEdgeSide(node.triangle, p, q)) {
      return;
    }
          ...
          while (it2.x < px) {
            if (
              orient2d(
                ...
                ((it2.next as PNode).next as PNode).point,
              ) == CCW
            ) {
              // Concave
          ...
          while (it2.x > px) {
            if (
              orient2d(
                ...
                ((it2.prev as PNode).prev as PNode).point,
              ) == CW
            ) {
              // Concave
  ...
  fillBasin(node_: PNode) {
    ...
    const basin_left_node =
      orient2d(node_.point, node_.next.point, node_.next.next.point) == CCW
        ? node_.next.next
        : node_.next;
    ...
    // No valid basin
    if (basin_bottom_node == basin_left_node) {
      return;
    }
    ...
    // No valid basins
    if (basin_right_node == basin_bottom_node) {
      return;
    }
    ...
    while (basin_width <= height) {
      ...
      if (node.prev == basin_left_node && node.next == basin_right_node) {
        ...
        if (
          orient2d(
            node.point,
            (node.next as PNode).point,
            ((node.next as PNode).next as PNode).point,
          ) == CW
        ) {
          return;
        }
        ...
        if (
          orient2d(
            node.point,
            (node.prev as PNode).point,
            ((node.prev as PNode).prev as PNode).point,
          ) == CCW
        ) {
          return;
        }
  ...
  fillRightConvexEdgeEvent(q: Point, p: Point, node: PNode) {
    while (node.next) {
      ...
      if (
        orient2d(
          node.next.point,
          (node.next.next as PNode).point,
          ((node.next.next as PNode).next as PNode).point,
        ) == CCW
      ) {
        // Concave
  ...
  flipEdgeEvent(ep: Point, eq: Point, t: Tri, p: Point, i: number) {
    ...
    if (inScanArea(p, t_p_ccw, t_p_cw, op)) {
      ...
      if (p == eq && op == ep) {
        if (
          eq == this._edge_event_constrained_edge_q &&
          ep == this._edge_event_constrained_edge_p
        ) {
