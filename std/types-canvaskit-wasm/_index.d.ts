export default function CanvasKitInit(): Promise<CanvasKit>;
...
export interface CanvasKit {
    ...
    readonly SkShader: SkShaderConstructor;
    ...
    readonly SkColorSpace: SkEnum<SkColorSpace, {
        SRGB: SkColorSpace;
        DISPLAY_P3: SkColorSpace;
        ADOBE_RGB: SkColorSpace;
    }>;
...
export interface SkImage extends SkObject<SkImage> {
    ...
    makeShader(tileModeX: SkTileMode, tileModeY: SkTileMode, localMatrix: SkMatrix): SkShader;
}
...
export interface SkCanvas extends SkObject<SkCanvas> {
    ...
    readPixels(x: number, y: number, w: number, h: number, alphaType?: SkAlphaType, colorType?: SkColorType, colorSpace?: SkColorSpace, dstRowBytes?: number): Uint8Array | undefined;
...
export interface SkShaderConstructor {
    MakeLinearGradient(start: SkPoint, end: SkPoint, colors: SkColor[], positions: number[], mode: SkTileMode, localMatrix: SkMatrix | null, flags: number): SkShader;

    MakeRadialGradient(center: SkPoint, radius: number, colors: SkColor[], positions: number[], mode: SkTileMode, localMatrix: SkMatrix | null, flags: number): SkShader;

    MakeTwoPointConicalGradient(start: SkPoint, startRadius: number, end: SkPoint, endRadius: number,
                                colors: SkColor[], positions: number[], mode: SkTileMode, localMatrix: SkMatrix | null, flags: number,): SkShader;
}
...
export type SkColorSpace = SkEnumValue;
