...
export class DisplayNinePatchComponent extends Display2DComponent {
    ...
    draw(drawer: Drawer) {
        ...
        drawer.state
            ...
            .saveMatrix().scale(1.0 / this.scale.x, 1.0 / this.scale.y);
    ...
    getBounds(out: Recta): void {
        const spr = this.sprite.data;
        if (spr !== null) {
            out.set(spr.rect.x - 1.0, spr.rect.y - 1.0, spr.rect.width + 2.0, spr.rect.height + 2.0);
        } else {
            out.setZero();
        }
    }
...
export const DisplayNinePatch = new ComponentTypeA(DisplayNinePatchComponent, Display2D);