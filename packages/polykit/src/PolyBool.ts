...
class EventNode {
    ...
    constructor(public isStart: boolean,
                public pt: Vector2,
                public seg: Segment,
...
function pointInsideRegion(x: number, y: number, region: Region): boolean {
    ...
    for (let i = 0; i < region.length; ++i) {
        ...
        if ((curr_y - y > EPSILON) != (last_y - y > EPSILON) && (last_x - curr_x) * (y - curr_y) / (last_y - curr_y) + curr_x - x > EPSILON) {
            inside = !inside;
        }
...
function linesIntersect(a0: Vector2, a1: Vector2, b0: Vector2, b1: Vector2, outResult: LineIntersectionResult) {
...
function operate(poly1: Polygon, poly2: Polygon, selector: (c: CombinedSegmentList) => SegmentList): Polygon {
...
class Intersecter {
    ...
    static eventLess(p1_isStart: boolean, p1_1: Vector2, p1_2: Vector2, p2_isStart: boolean, p2_1: Vector2, p2_2: Vector2): boolean {
        // compare the selected points first
        // returns -1 if p1 is smaller, 1 if p2 is smaller, 0 if equal
        if (p1_1 != p2_1) {
            if (Math.abs(p1_1.x - p2_1.x) < EPSILON) {
                if (Math.abs(p1_1.y - p2_1.y) < EPSILON) {
...
// 		if (dx > -PolyBool.EPSILON && dx < PolyBool.EPSILON && dy > -PolyBool.EPSILON && dy < PolyBool.EPSILON) {
    ...
    checkBothIntersections(ev: EventNode, above: EventNode | null, below: EventNode | null): EventNode | null {
        if (above != null) {
...
// 				}
                ...
                ev.seg.fill = fill_setBelow(ev.seg.fill,
    ...
    calculateNoSelfIntersection(segments1: Segment[], inverted1: boolean, segments2: Segment[], inverted2: boolean): Segment[] {
    ...
    addRegion(region: Region) {
        ...
        for (let i = 0; i < region.length; ++i) {
            ...
            this.eventAddSegment(new Segment(forward < 0 ? pt1 : pt2, forward < 0 ? pt2 : pt1, FILL_NULL), true);
        }
    }
    ...
    insertNodeBefore(ev: EventNode, other_pt: Vector2) {
        ...
        const p1_1 = ev.pt;
        while (here != null) {
            if (Intersecter.eventLess(
                p1_isStart, p1_1, other_pt,
