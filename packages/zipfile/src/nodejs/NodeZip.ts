...
export class NodeZip extends Zip {
     ...
     * @return Boolean
     */
    extractEntryTo(entry: string | ZipEntry, targetPath: string, maintainEntryPath: boolean = true, overwrite: boolean = false) {
        ...
        let target = sanitize(targetPath, maintainEntryPath ? entryName : path.basename(entryName));
        ...
        if (item.isDirectory) {
            ...
            for (const child of this._zip.getChildren(item)) {
                ...
                const childName = sanitize(targetPath, maintainEntryPath ? child.entryName : path.basename(child.entryName));
     ...
     *                  Default is FALSE
     */
    extractAllTo(targetPath: string, overwrite: boolean = false) {
        if (!this._zip) {
