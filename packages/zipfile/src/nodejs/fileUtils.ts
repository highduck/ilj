...
function findSync(dir: string, pattern?: RegExp, recursive: boolean = false) {
    let files: string[] = [];
...
export function makeDir(p: string) {
    mkdirSync(p);
}

export function writeFileTo(p: string, content: Uint8Array, overwrite: boolean, attr: number = 438) {
    if (fs.existsSync(p)) {
        if (!overwrite) {
...
export function writeFileToAsync(p: string, content: Uint8Array, overwrite: boolean, attr: undefined | number): Promise<boolean> {
...
export function findFiles(p: string) {
