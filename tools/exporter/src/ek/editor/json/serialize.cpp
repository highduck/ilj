...
json to_json(const float2& v, int precision = 4) {
    return json::array({
                               setprecision(v.x, precision),
                               setprecision(v.y, precision)
                       });
}
...
json to_json(const std::vector<float2>& points) {
    ...
    for (const auto& point : points) {
        r.emplace_back(setprecision(point.x, 6));
        r.emplace_back(setprecision(point.y, 6));
    }
    return r;
}
...
json to_json(const std::vector<movie_frame_data>& frames) {
    ...
    for (const auto& frame : frames) {
        json e = {
                {"i",   frame.index},
                {"len", frame.duration},
                //{"mot", frame.motion_type},
                {"key", frame.key}
        };
        if (frame.position != float2::zero) {
            e["p"] = to_json(frame.position, 2);
        }
        if (frame.scale != float2::one) {
            e["s"] = to_json(frame.scale);
        }
        if (frame.skew != float2::zero) {
            e["r"] = to_json(frame.skew);
        }
        if (frame.pivot != float2::zero) {
            e["o"] = to_json(frame.pivot);
        }
        if (frame.color.multiplier != float4::one) {
            e["cm"] = to_json(frame.color.multiplier);
        }
        if (frame.color.offset != float4::zero) {
            e["co"] = to_json(frame.color.offset);
        }
...
json to_json(const sg_node_data& node) {
    ...
    if (node.color.multiplier != float4::one) {
        r["cm"] = to_json(node.color.multiplier);
    }
    if (node.color.offset != float4::zero) {
        r["co"] = to_json(node.color.offset);
    }
    // matrix
    if (node.button) {
