...
public class Splash {
  ...
  private final static int IMMERSIVE_MODE_SYSTEM_UI_FLAGS = View.SYSTEM_UI_FLAG_LAYOUT_STABLE
  ...
  private static void buildViews(Context c) {
    final boolean splashFullScreen = Config.getBoolean(CONFIG_KEY_PREFIX + "splashFullScreen", DEFAULT_SPLASH_FULL_SCREEN);
    final boolean splashImmersive = Config.getBoolean(CONFIG_KEY_PREFIX + "splashImmersive", DEFAULT_SPLASH_IMMERSIVE);
    ...
    if (spinnerBar == null) {
      ...
      spinnerBar.setFitsSystemWindows(true);

      // Hide status bar during splash screen.
      if(splashFullScreen) {
        splashImage.setSystemUiVisibility(View.SYSTEM_UI_FLAG_FULLSCREEN);
        spinnerBar.setSystemUiVisibility(View.SYSTEM_UI_FLAG_FULLSCREEN);
      }

      // Enable immersive mode (hides status bar and navbar) during splash screen.
      if (splashImmersive) {
        splashImage.setSystemUiVisibility(IMMERSIVE_MODE_SYSTEM_UI_FLAGS);
        spinnerBar.setSystemUiVisibility(IMMERSIVE_MODE_SYSTEM_UI_FLAGS);
      }
  ...
  public static void showOnLaunch(final BridgeActivity a) {
    int duration = Config.getInt(CONFIG_KEY_PREFIX + "launchShowDuration", DEFAULT_LAUNCH_SHOW_DURATION);
  ...
  public static void show(final Activity a,
    ...
    if(isLaunchSplash) {
      runShow(a, listener, fadeInDuration);
    }
    else {
      // `postAtFrontOfQueue` to avoid short blink when switching from windowBackground to splash-image view
      new Handler(a.getMainLooper()).postAtFrontOfQueue(
              () -> runShow(a, listener, fadeInDuration)
      );
    }
  }

  private static void runShow(final Activity a,
    ...
    if(fadeInDuration > 0) {
      ...
      splashImage.setAlpha(1f);
        ...
        if (spinnerBar != null) {
          final boolean showSpinner = Config.getBoolean(CONFIG_KEY_PREFIX + "showSpinner", false);
