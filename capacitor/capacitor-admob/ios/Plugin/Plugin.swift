...
let BANNER_VIEW_TAG = 2743243288699
...
public class AdMob: CAPPlugin, GADBannerViewDelegate, GADInterstitialDelegate, GADRewardedAdDelegate {
    ...
    @objc func hideBanner(_ call: CAPPluginCall) {
        DispatchQueue.main.async {
            if let rootViewController = UIApplication.shared.keyWindow?.rootViewController {
                if let subView = rootViewController.view.viewWithTag(BANNER_VIEW_TAG) {
    ...
    private func addBannerViewToView(_ bannerView: GADBannerView, _ adPosition: String, _ Margin: String) {
        ...
        if let rootViewController = UIApplication.shared.keyWindow?.rootViewController {
            ...
            bannerView.tag = BANNER_VIEW_TAG // rand
    ...
    /// the App Store), the current app moving to background.
        ...
        DispatchQueue.main.async {
            ...
            self.rewardedAd?.load(GADRequest()) { error in
                if let error = error {
                    ...
                    self.notifyListeners("onRewardedVideoAdFailedToLoad", data: ["value": true])
                } else {
